...
export default function EmailRecoveryPage() {
  ...
  const onSendCode = async (data: EmailFormData) => {
    ...
    try {
      const response = await fetchWithCsrf("/api/auth/password-reset/send-email-code", {
        method: "POST",
  ...
  const onVerifyCode = async (data: CodeFormData) => {
    ...
    try {
      const response = await fetchWithCsrf("/api/auth/password-reset/verify-email-code", {
        method: "POST",
      ...
      console.log('🔐 Verification successful, received token:', result.resetToken);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-1">邮箱找回密码</h1>
        ...
        {step !== "success" && (
          <div className="flex justify-center gap-2 mb-6">
            <div className={`h-1 rounded-full transition-all duration-300 ${
              step === "email" ? "w-full bg-purple-500" : "w-1/3 bg-purple-500"
            }`} />
            <div className={`h-1 rounded-full transition-all duration-300 ${
